...
export default function EditReferencePage() {
    ...
    const fetchReference = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error('Chyba při načítání reference');
        }
        ...
        setFormData({
          ...
          published: referenceData.published,
        });
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Chyba při ukládání reference');
      }
  ...
  if (!reference) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600">Reference nebyla nalezena</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-gray-700">
                Úprava reference: {reference.title}
              </p>
            </div>
          </div>
        </div>
