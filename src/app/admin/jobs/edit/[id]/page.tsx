...
export default function EditJobPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Chyba při aktualizaci pozice');
      }
  ...
  const handleDelete = async () => {
    if (!confirm('Opravdu chcete smazat tuto pracovní pozici? Tato akce se nedá vrátit.')) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Chyba při mazání pozice');
      }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Aktualizujte informace o pracovní pozici
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 p-6">
            ...
            <div className="flex justify-between items-center pt-6 border-t border-gray-200 dark:border-zinc-700">
              <button
                ...
                disabled={deleting}
                className="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 disabled:bg-gray-400 text-white font-medium rounded-md transition-colors duration-200"
              >
                {deleting ? (
